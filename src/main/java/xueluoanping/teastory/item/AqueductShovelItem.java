    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (blockState.is(Tags.Blocks.COBBLESTONE)) {
            if (!world.isClientSide()) {
                world.setBlock(blockPos, ((AqueductBlock) ModContents.cobblestoneAqueduct.get()).getStateForPlacement(world, blockPos), 3);
                world.scheduleTick(blockPos, ModContents.cobblestoneAqueduct.get(), Fluids.WATER.getTickDelay(world));
            ...
            if (!world.isClientSide()) {
                world.setBlock(blockPos, ((AqueductBlock) ModContents.mossyCobblestoneAqueduct.get()).getStateForPlacement(world, blockPos), 3);
                world.scheduleTick(blockPos, ModContents.mossyCobblestoneAqueduct.get(), Fluids.WATER.getTickDelay(world));
                if (playerEntity != null) {
                    context.getItemInHand().hurtAndBreak(1, playerEntity, player -> player.broadcastBreakEvent(context.getHand()));
                }
            }
