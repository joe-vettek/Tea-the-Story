        ...
        if (worldIn.isClientSide()) {
            ...
            if (player.mayUseItemAt(pos, context.getClickedFace(), itemstack) && (flag || worldIn.isEmptyBlock(pos)) && worldIn.getBlockState(pos.below()).isFaceSturdy(worldIn, pos.below(), Direction.UP)) {
                ...
                if (left || right) {
                    ...
                    BlockState iblockstate2 = BlockEntityRegister.DRINK_MAKER.get().defaultBlockState().setValue(DrinkMakerBlock.LEFT, left).setValue(DrinkMakerBlock.FACING, enumfacing);
