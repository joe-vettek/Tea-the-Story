...
public class HybridizableFlowerBlockItem extends BlockItem {
    ...
    public void appendHoverText(ItemStack itemStack, TooltipContext context, TooltipDisplay display, Consumer<Component> builder, TooltipFlag tooltipFlag) {
        ...
        if (itemStack.has(DataComponents.BLOCK_STATE)) {
            builder.accept(Component.translatable(itemStack.get(DataComponents.BLOCK_STATE).get(HybridizableFlowerBlock.FLOWER_COLOR).getTranslation()).withStyle(ChatFormatting.ITALIC).withStyle(ChatFormatting.GRAY));
        }
    }
